...
type StdNetBind struct {
	...
	blackhole4          bool
	blackhole6          bool
	reservedForEndpoint map[netip.AddrPort][3]byte
}
...
func NewStdNetBind(listener ...Listener) Bind {
	...
	return &StdNetBind{
		...
		reservedForEndpoint: make(map[netip.AddrPort][3]byte),
	}
}

// SetReservedForEndpoint sets the reserved bytes for a specific endpoint.
func (s *StdNetBind) SetReservedForEndpoint(destination netip.AddrPort, reserved [3]byte) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.reservedForEndpoint == nil {
		s.reservedForEndpoint = make(map[netip.AddrPort][3]byte)
	}
