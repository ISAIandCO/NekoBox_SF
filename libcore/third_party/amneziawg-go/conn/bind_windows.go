...
import (
	...
	"github.com/pulsarvpn/amneziawg-go/conn/winrio"
)
...
// SetReservedForEndpoint is a no-op for WinRingBind as it doesn't support per-endpoint reserved bytes
func (bind *WinRingBind) SetReservedForEndpoint(destination netip.AddrPort, reserved [3]byte) {
	// Windows RIO implementation doesn't modify packet headers for reserved bytes
}
