...
import (
	...
	"github.com/pulsarvpn/amneziawg-go/ratelimiter"
	"github.com/pulsarvpn/amneziawg-go/rwcancel"
	"github.com/pulsarvpn/amneziawg-go/tun"
)
...
func NewDevice(args ...any) *Device {
	var tunDevice tun.Device
	var bind conn.Bind
	var logger *Logger
	cpus := runtime.NumCPU()

	// Native amneziawg-go signature:
	//   NewDevice(tun.Device, conn.Bind, *Logger)
	if len(args) >= 3 {
		if td, ok := args[0].(tun.Device); ok {
			tunDevice = td
			if b, ok := args[1].(conn.Bind); ok {
				bind = b
			}
	...
	// SagerNet wireguard-go signature (used by sing-box transport):
	//   NewDevice(context.Context, tun.Device, conn.Bind, *Logger, workers)
	if tunDevice == nil && len(args) >= 5 {
		if td, ok := args[1].(tun.Device); ok {
			tunDevice = td
		}
		if b, ok := args[2].(conn.Bind); ok {
			bind = b
		}
		if l, ok := args[3].(*Logger); ok {
			logger = l
		}
		if workers, ok := args[4].(int); ok && workers > 0 {
			cpus = workers
		}
	}

	if tunDevice == nil || bind == nil || logger == nil {
		panic("wireguard/device: invalid NewDevice arguments")
	}
	...
	for _, extra := range args {
		if workers, ok := extra.(int); ok && workers > 0 {
			cpus = workers
		}
	}
