...
+ 260808 - Профиль "Самый быстрый" теперь можно наполнять не только самостоятельно (добавляя нужные прокси руками), но и динамически через regex. Примеры: Только НЕ Российские - "^(?!.*(🇷🇺|Россия)).*$", только Российские - "^(.*(🇷🇺|Россия)).*$", только Германия\США\Болгария - "^(.*(🇩🇪|Германия|🇺🇸|США|🇧🇬|Болгария)).*$".
...
## Минусы
+ Кажется сломалась *маршрутизация* **при "Персональном DNS"**. Она влияет только на обходные маршруты, так что если нужна - отключить персональный.
...
## Рекомендации (уровня ИМХО)
   ...
   - [x] "Режим VPN для приложений" (обход > выбрать не нужные или прокси > выбрать нужные)
   - [x] "Обход LAN"
   - [x] "Обход LAN в ядре"
   - [x] "Определить адрес назначения"
   - [x] "RuleSet Update Interval" > 72h
   - [x] Удаленный\Прямой DNS (указать DoH, например https://dns.adguard-dns.com/dns-query или https://1.1.1.1/dns-query)
   - [x] "Правило домена..." - prefer_ipv4 (ipv6 хорошо, но не в России - не распространено)
   - [ ] "Включить маршрутизацию DNS" (вклбчать только при выключенном персональном dns)
   - [ ] "Включить FakeDNS"
